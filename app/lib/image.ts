...
export const draw = (obj:MyDraw) => {
    ...
    const backCtx = obj.back.getContext('2d')
    if(!ctx || !backCtx) return obj.scale ?? 1
    let pos = obj.pos
    if(!pos) pos = [0, 0]
    ...
    for(const c of [ctx, backCtx]){
        c.setTransform(1, 0, 0, 1, 0, 0)
        c.clearRect(0, 0, width, height)
        c.setTransform(scale, 0, 0, scale, pos[0] * scale + width / 2, pos[1] * scale + height / 2)
    }
    const pa = backCtx.createPattern(obj.pattern, 'repeat')
    backCtx.fillStyle = pa ?? 'white'
    backCtx.fillRect(- width / 2 / scale - pos[0], - height / 2 / scale - pos[1], width / scale, height / scale)
